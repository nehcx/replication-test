...
Code source:
https://github.com/a1da4/sppmi-svd/blob/d1648f59a650caafec2f3de7ac30c9aed2a87e75/util.py
"""
...
def sppmi(C, k, eps=1e-8, has_abs_dis=False, has_cds=False):
    ...
    if has_abs_dis:
        ...
        size = len(C) * len(C[0])
        C_each_morethan1 = [
            sum([1 if c >= 1 else 0 for c in C_each]) for C_each in C
        ]
        C_each_morethan2 = [
            sum([1 if c >= 2 else 0 for c in C_each]) for C_each in C
        ]
